...
const NavSheet = () => {
  const [messages, setSMessage] = useState<Message[] | null>(null);
  useEffect(() => {
    const newMessages = window.localStorage.getItem("chat");
    if (newMessages) {
      const streamMessage = JSON.parse(newMessages);
      setSMessage(streamMessage);
    }
  }, []);
...
//   console.log(messages);
  return (
    <Sheet>
      <SheetTrigger>
        <span className="absolute mt-1 hidden max-lg:flex ml-8 bg-background top-[25%] left-[-5%] xs:left-[0%]">
          ...
          <SheetTitle className="-mt-3">
            <div className="flex items-center justify-between w-full">
              <div className="flex items-center gap-2">
                ...
                <span className="text-base text-black dark:text-white/90 font-bold">
                  New chat
                </span>
              </div>
              <SquarePen
                size={20}
                className="text-gray-600 dark:text-white/90 cursor-pointer"
              />
            </div>
          </SheetTitle>
          <SheetDescription>
